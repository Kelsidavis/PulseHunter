        ...
        if folder_key in self.projects:
            ...
            for cal_type, file_path in master_files.items():
                if Path(file_path).exists():
                    ...
                    self.logger.debug(f"Found master {cal_type}: {Path(file_path).name}")
                else:
                    self.logger.warning(f"Master {cal_type} file missing: {file_path}")
            
            if verified_masters:
                self.logger.info(f"Found {len(verified_masters)} master files for {Path(lights_folder).name}")
        ...
        if folder_key in self.filter_projects:
            ...
            if filter_masters:
                ...
                for cal_type, file_path in masters.items():
                    if Path(file_path).exists():
                        verified_masters[cal_type] = file_path
                        self.logger.debug(f"Found filter master {cal_type}: {Path(file_path).name}")
                
                if verified_masters:
                    self.logger.info(f"Found {len(verified_masters)} filter-aware master files")
                    return verified_masters
...
def enhanced_load_fits_stack(
    ...
    if auto_calibrate and (master_bias is None or master_dark is None or master_flat is None):
    ...
    def load_single_fits(fits_file: Path) -> Optional[Tuple[np.ndarray, str, Optional[WCS]]]:
            ...
            if master_bias is not None:
                try:
                    if master_bias.shape == data.shape:
                        ...
                        logger.warning(f"Bias frame shape mismatch for {fits_file.name}")
                except Exception as e:
            ...
            if master_dark is not None:
                try:
                    if master_dark.shape == data.shape:
                        ...
                        logger.warning(f"Dark frame shape mismatch for {fits_file.name}")
                except Exception as e:
    ...
    if cal_summary:
        logger.info(f"✅ Loaded {len(frames)} frames with {', '.join(cal_summary)} calibration")
    else:
...
def apply_calibration_to_image(
    ...
    if master_bias is not None:
        if not validate_shapes or master_bias.shape == calibrated.shape:
            ...
            logger.warning(f"Bias shape mismatch: {master_bias.shape} vs {calibrated.shape}")
    ...
    if master_dark is not None:
        if not validate_shapes or master_dark.shape == calibrated.shape:
            ...
            logger.warning(f"Dark shape mismatch: {master_dark.shape} vs {calibrated.shape}")
    ...
    if master_flat is not None:
        if not validate_shapes or master_flat.shape == calibrated.shape:
            ...
            logger.warning(f"Flat shape mismatch: {master_flat.shape} vs {calibrated.shape}")
