...
from calibration_utilities import CalibrationLogger, CalibrationConfig
...
class CalibrationProcessor:
    ...
    def create_master_calibration(self, 
        ...
        """
        self.logger.info(f"Creating master {calibration_type} from {len(input_files)} files")
        ...
        try:
            ...
            for i, file_path in enumerate(input_files):
                if progress_callback:
                    progress_callback(int((i / len(input_files)) * 50))  # First 50% for reading
            ...
            if not self._validate_frame_consistency(valid_files, calibration_type):
                self.logger.warning("Frame consistency validation failed, proceeding anyway")
            ...
            # Save master frame
            success = self._save_master_frame(master_data, output_file, calibration_type, valid_files)
            
            if progress_callback:
                progress_callback(100)
            
            if success:
                self.logger.info(f"Master {calibration_type} created successfully: {output_file}")
    ...
    def _validate_frame_consistency(self, files: List[Path], calibration_type: str) -> bool:
        ...
        for file_path in files[1:]:
            ...
            if fits_info['dimensions'] != ref_dimensions:
                self.logger.warning(f"Dimension mismatch in {file_path.name}: {fits_info['dimensions']} vs {ref_dimensions}")
                inconsistent_count += 1
            ...
            if calibration_type.lower() in ['dark', 'dark_flat']:
                ...
                if exp_diff > self.config.getfloat('VALIDATION', 'max_exposure_variance', 0.1):
                    self.logger.warning(f"Exposure time mismatch in {file_path.name}: {fits_info['exposure_time']}s vs {ref_exposure}s")
                    inconsistent_count += 1
            
            # Check binning
            if fits_info['binning'] != ref_binning:
                self.logger.warning(f"Binning mismatch in {file_path.name}: {fits_info['binning']} vs {ref_binning}")
                inconsistent_count += 1
    ...
    def _combine_frames(self, frames_data: List[np.ndarray], calibration_type: str) -> np.ndarray:
        """Combine multiple frames into master frame"""
        combination_method = self.config.get('PROCESSING', 'combination_method', 'median')
        
        self.logger.info(f"Combining {len(frames_data)} frames using {combination_method} method")
        ...
        if combination_method == 'median':
            ...
            if ASTROPY_AVAILABLE:
                sigma_threshold = self.config.getfloat('PROCESSING', 'sigma_clipping_threshold', 3.0)
            ...
            self.logger.warning(f"Unknown combination method: {combination_method}, using median")
            master_frame = np.median(frame_stack, axis=0)
    ...
    def _save_master_frame(self, data: np.ndarray, output_file: Path, 
                          calibration_type: str, input_files: List[Path]) -> bool:
        """Save master frame to FITS file"""
        try:
            output_file.parent.mkdir(parents=True, exist_ok=True)
            
            if ASTROPY_AVAILABLE:
                return self._save_with_astropy(data, output_file, calibration_type, input_files)
            else:
                return self._save_simulated(data, output_file, calibration_type, input_files)
                
        except Exception as e:
            self.logger.error(f"Error saving master frame: {e}")
            return False
    
    def _save_with_astropy(self, data: np.ndarray, output_file: Path,
                          calibration_type: str, input_files: List[Path]) -> bool:
        """Save using astropy"""
        # Create header
        header = fits.Header()
        ...
        header['COMBMETH'] = self.config.get('PROCESSING', 'combination_method', 'median')
    ...
    def _save_simulated(self, data: np.ndarray, output_file: Path,
                       calibration_type: str, input_files: List[Path]) -> bool:
        ...
        try:
            ...
            if data.size > 0:
                ...
                if dead_pixel_ratio > 0.05:  # More than 5% dead pixels
                    quality['issues'].append(f'Many dead pixels: {dead_pixel_ratio:.1%}')
                    score -= 10
...
def create_master_bias(input_folder: Path, output_file: Path, progress_callback=None) -> bool:
    """Create master bias frame"""
    processor = CalibrationProcessor()
    bias_files = list(input_folder.glob("*bias*.fit*")) + list(input_folder.glob("*BIAS*.fit*"))
    return processor.create_master_calibration(bias_files, output_file, 'bias', progress_callback)

def create_master_dark(input_folder: Path, output_file: Path, progress_callback=None) -> bool:
    """Create master dark frame"""
    processor = CalibrationProcessor()
    dark_files = list(input_folder.glob("*dark*.fit*")) + list(input_folder.glob("*DARK*.fit*"))
    return processor.create_master_calibration(dark_files, output_file, 'dark', progress_callback)

def create_master_flat(input_folder: Path, output_file: Path, progress_callback=None) -> bool:
    """Create master flat frame"""
    processor = CalibrationProcessor()
    flat_files = list(input_folder.glob("*flat*.fit*")) + list(input_folder.glob("*FLAT*.fit*"))
    return processor.create_master_calibration(flat_files, output_file, 'flat', progress_callback)
