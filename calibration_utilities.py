...
class DialogPositionManager:
    ...
    def save_dialog_geometry(dialog, settings_key):
        ...
        settings.setValue(f"{settings_key}/state", dialog.saveState() if hasattr(dialog, 'saveState') else None)
...
class ASTAPManager:
    ...
    def auto_detect_astap(self):
        ...
        if platform.system() == "Windows":
            ...
            for user_location in [Path.home() / "astap", Path.home() / "Desktop", Path.home() / "Downloads"]:
                for cli_name in cli_names:
        ...
        self.logger.info(f"Checking {len(potential_paths)} potential ASTAP locations...")
        ...
        for path in potential_paths:
            ...
            if path_obj.exists():
                ...
                self.logger.info(f"Auto-detected ASTAP {app_type} version (validation skipped): {path_obj}")
                return str(path_obj)
    ...
    def validate_astap_executable(self, path=None):
        ...
        is_cli_version = any(indicator in filename_lower for indicator in ['cli', 'console', 'cmd'])
        ...
        if "astap" not in filename_lower:
            self.logger.warning(f"Executable name doesn't contain 'astap': {path_obj.name}")
        ...
        for args in test_commands:
            try:
                ...
                if result.stdout or result.stderr:
                    ...
                    if any(keyword in output_text for keyword in ['astap', 'astrometric', 'plate', 'solve', 'help', 'usage', 'version']):
                        self._is_validated = True
    ...
    def _validate_gui_version(self, check_path):
        ...
        self.logger.info("GUI ASTAP validation - assuming valid based on file existence")
        self._is_validated = True
        return True
    ...
    def run_astap_command(self, args, timeout=None):
        ...
        additional_params = self.config.get('ASTAP', 'additional_parameters', '').split()
        ...
        try:
            ...
            if result.returncode != 0:
                self.logger.warning(f"ASTAP returned non-zero exit code: {result.returncode}")
    ...
    def get_status_info(self):
        ...
        try:
            ...
            return {
                'configured': True,
                'valid': is_valid,
                'message': 'ASTAP ready' if is_valid else 'ASTAP found but validation failed',
...
class FITSFileValidator:
    ...
    def validate_files(self, file_list, calibration_type):
        ...
        self.logger.info(f"Validating {len(file_list)} files for {calibration_type} calibration")
        ...
        if validation_results['valid_files']:
            self._check_consistency(validation_results, exposure_times, temperatures, binning_modes)
    ...
    def _simulate_fits_header(self, file_path, calibration_type):
        ...
        return {
            'valid': True,
            'exposure': base_exposure.get(calibration_type, 1.0) + random.uniform(-0.05, 0.05),
    ...
    def _check_consistency(self, results, exposures, temperatures, binning):
        ...
        if exposures:
            ...
            max_variance = self.config.getfloat('VALIDATION', 'max_exposure_variance', 0.1)
        ...
        if temperatures:
            ...
            max_temp_variance = self.config.getfloat('VALIDATION', 'max_temperature_variance', 2.0)
...
class CalibrationFileManager:
    ...
    def save_processing_log(self, calibration_type, processing_info):
        ...
        log_file = self.base_path / f"processing_log_{calibration_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
class ProgressReporter:
    ...
    def get_current_description(self):
        return self.step_descriptions.get(self.current_step, f"Step {self.current_step}")
...
def setup_calibration_environment():
    ...
    output_base = Path(config.get('DIALOG', 'default_output_folder', 'calibration_output'))
