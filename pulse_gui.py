    ...
    def run(self):
        ...
        try:
            ...
            self.log_updated.emit(
                f"Analyzing images with threshold z={detection_threshold}"
            )
            ...
            print(
                f"🔧 EMERGENCY: Emitting completion signal with {len(detections)} detections"
            )
            ...
            if not hasattr(self, "_cleanup_signal_sent"):
                ...
                try:
                    ...
                    self.processing_finished.emit(
                        True, "Processing completed (cleanup safety net)", []
                    )
...
class DetectionResultsWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.type_combo.addItems(
            ["All", "Brightening", "Dimming", "GAIA Matched", "Exoplanet Candidates"]
        )
    ...
    def populate_table(self):
        ...
        for row, detection in enumerate(filtered_detections):
            ...
            self.results_table.setItem(
                row, 0, QTableWidgetItem(str(detection.get("frame", "")))
            )
            ...
            if match_name and "GAIA" in match_name:
                match_text = (
                    f"GAIA ({detection.get('angular_distance_arcsec', 0):.1f}\")"
                )
            else:
    ...
    def get_filtered_detections(self):
        ...
        for detection in self.detections:
            ...
            if filter_type == "Brightening" and detection.get("dimming"):
                ...
                continue
            elif filter_type == "Exoplanet Candidates" and not detection.get(
                "exo_match"
            ):
                continue
...
GAIA: {detection.get('match_name', 'None')}
    ...
    def plot_light_curve(self, detection):
        ...
        if det_frame < len(light_curve):
            ax.plot(
                det_frame, light_curve[det_frame], "ro", markersize=8, label="Detection"
            )
...
class PulseHunterMainWindow(QMainWindow):
    ...
    def initialize_astap(self):
        ...
        if (
            self.config.getboolean("ASTAP", "auto_detect_on_startup", True)
            and not self.astap_manager.astap_path
        ):
    ...
    def update_astap_status(self):
        ...
        if status_info["valid"]:
            self.astap_status_label.setText(
                f"ASTAP: Ready ({Path(status_info['path']).name})"
            )
            self.astap_status_label.setStyleSheet("color: green; padding: 0 10px;")
            self.astap_status_label.setToolTip(
                f"ASTAP Path: {status_info['path']}\n{status_info['version']}"
            )
            ...
            self.astap_status_label.setStyleSheet("color: #666; padding: 0 10px;")
            self.astap_status_label.setToolTip(
                "ASTAP executable not configured. Use Calibration > Configure ASTAP"
            )
    ...
    def update_detection_count(self):
        ...
        if count > 0:
            high_conf = sum(
                1 for d in self.current_detections if d.get("confidence", 0) > 0.8
            )
            if high_conf > 0:
                self.detection_count_label.setStyleSheet(
                    "color: green; padding: 0 10px; font-weight: bold;"
                )
                self.detection_count_label.setToolTip(
                    f"Total: {count} detections\nHigh confidence: {high_conf}"
                )
            else:
                self.detection_count_label.setStyleSheet(
                    "color: orange; padding: 0 10px;"
                )
                self.detection_count_label.setToolTip(
        ...
        if file_path:
            try:
                ...
                QMessageBox.critical(
                    self, "Error Loading Project", f"Could not load project:\n{str(e)}"
                )
    ...
    def save_project(self):
        ...
        if not self.current_project:
            QMessageBox.information(
                self, "No Project", "No project to save. Create a new project first."
            )
            return

        file_path, _ = QFileDialog.getSaveFileName(
            self,
            "Save PulseHunter Project",
            "",
            ...
            try:
                ...
                with open(file_path, "w") as f:
                    if file_path.endswith(".phproj"):
                        ...
                        json.dump(
                            self.current_project, f, indent=2, default=convert_numpy
                        )
                ...
                QMessageBox.critical(
                    self, "Error Saving Project", f"Could not save project:\n{str(e)}"
                )
    ...
    def update_project_status(self):
        ...
        if self.current_project:
            ...
            exo_candidates = sum(
                1 for d in self.current_detections if d.get("exo_match")
            )
    ...
    def configure_astap(self):
        ...
        def browse_astap():
            ...
            if file_path:
                ...
                if self.astap_manager.validate_astap_executable(file_path):
                    ...
                    QMessageBox.information(
                        dialog, "Success", "ASTAP configured successfully!"
                    )
                else:
                    QMessageBox.warning(
                        dialog,
                        "Validation Failed",
                        "Selected file failed ASTAP validation.",
                    )
        ...
        def auto_detect():
            ...
            if detected:
                ...
                QMessageBox.information(
                    dialog, "Success", f"ASTAP auto-detected at:\n{detected}"
                )
            else:
                QMessageBox.information(
                    dialog, "Not Found", "Could not auto-detect ASTAP executable."
                )
    ...
    def process_images(self):
        ...
        self.add_system_log(
            "Image processing started with emergency timeout protection"
        )
    ...
    def processing_finished(self, success, message, detections):
        ...
        try:
            print(
                f"Processing finished: success={success}, detections={len(detections)}"
            )
            ...
            if success:
                ...
                QTimer.singleShot(
                    500,
                    lambda: self.show_success_message_safe(message, len(detections)),
                )
        ...
        try:
            ...
            if detection_count > 0:
                self.status_bar.showMessage(
                    f"✅ Processing Complete! Found {detection_count} detections", 10000
                )
                ...
                self.status_bar.showMessage(
                    "✅ Processing Complete - No detections found", 5000
                )
    ...
    def export_csv(self):
        ...
        if file_path:
            try:
                ...
                QMessageBox.critical(
                    self, "Export Error", f"Could not export results:\n{str(e)}"
                )
    ...
    def show_about(self):
        """Show about dialog"""
        astap_status = (
            "✓ Configured" if self.astap_manager.is_configured() else "✗ Not configured"
        )
        det_count = len(self.current_detections)
    ...
    def show_success_message_safe(self, message, detection_count):
        ...
        try:
            ...
            self.status_bar.showMessage(
                f"✅ Found {detection_count} detections! Check Results tab.", 10000
            )
            print(f"Processing complete: {detection_count} detections found")

            # Log to system log
            self.add_system_log(
                f"Processing complete: {detection_count} detections found"
            )
    ...
    def show_error_message_safe(self, message):
        ...
        try:
            self.status_bar.showMessage(
                f"❌ Processing failed: {message[:50]}...", 10000
            )
    ...
    def emergency_processing_complete(self):
            ...
            for results_file in results_files:
                if os.path.exists(results_file):
                    ...
                    with open(results_file, "r") as f:
                        ...
                        print(
                            f"🚨 EMERGENCY: Loaded {len(detections)} detections from {results_file}"
                        )
                        break
        except Exception as e:
        ...
        self.processing_finished(
            True,
            f"Emergency completion: {len(detections)} detections found",
            detections,
        )
