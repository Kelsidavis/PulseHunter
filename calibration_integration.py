...
def _basic_fits_loading_fallback(folder, **kwargs):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        results = list(
            filter(None, executor.map(load_fits_file, fits_files[:5]))
        )  # Limit to 5 for fallback
...
def get_calibration_info(lights_folder: str) -> Dict:
    ...
    if info["calibration_status"] == "not_configured":
        ...
        info["recommendations"].append(
            f"✅ Calibration configured ({available_count} master files)"
        )
...
def migrate_calibration_projects():
    ...
    old_files = ["calibration_projects.json", "filter_calibration_projects.json"]
